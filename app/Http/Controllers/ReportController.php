...
class ReportController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($contacts)
                ->editColumn('name', function ($row) {
                    ...
                    $account=AccountingAccount::where('contact_id',$row->id)->first();
                    if(!empty($account)){
                        return '<a href="/accounting/ledger/'.$account->id.'" target="_blank" class="no-print">'.
                            $name.
                        '</a>';
                    }
                    return $name;
                ...
                ->addColumn('due', function ($row) {
                   ...
                   /*
                    ...
                    $due_details = $this->transactionUtil->getLedgerDetails($row->id,'2020-01-01',$end_date);
                    //$due_details = $this->transactionUtil->getLedgerDetails($row->id,'2020-01-01',$end_date);
                    $due=0;
                  
                    if(!empty($due_details)){
                        $due=$due_details['balance_due'] ?? 0;
                    }
                   */
                    ...
                    if(!empty($account)){
                        ...
                        ->where(function ($query) use ($start_date,$end_date, $row) {
                            $query->where('operation_date', '<', $end_date)
                                  ->orWhere(function ($subQuery) use ($row,$end_date) {
                                      $subQuery->where('operation_date', '=', $end_date)
                                               ->where('id', '<=', $row->id); // Ensure correct ordering for same date
                                  });
                        })
     ...
     * @return \Illuminate\Http\Response
     */
    private function getUnitData($row) {
        $unit_data = [];
    
        if ($row->enable_stock == 1) {
            $stock = (float)$row->stock;
            $baseUnit = \DB::table('units')->where('id', $row->unit_id)->first();
    
            // If there are sub-units
            $sub_unit_ids = !empty($row->sub_unit_ids) ? json_decode($row->sub_unit_ids, true) : [];
            $all_units = \DB::table('units')
                ->whereIn('id', array_merge( $sub_unit_ids)) // Include base unit
                ->orderBy('base_unit_multiplier', 'asc')
                ->get();
    
            foreach ($all_units as $k => $unit) {
                $multiplier = (float)$unit->base_unit_multiplier ?: 1;
                $unit_qty = $stock / $multiplier;
                $unit_data[$k]['f'] = $this->transactionUtil->num_f($unit_qty, false) . ' ' . $unit->short_name;
                $unit_data[$k]['uf']=$unit_qty;
            }
            if(empty($unit_data)){
                $unit_data[0]['f'] = $this->transactionUtil->num_f($stock, false) . ' '. $baseUnit->short_name;
                $unit_data[0]['uf']=$stock;
            }
        }
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'qty_status','category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type',
                'only_mfg_products','categories_ids','active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state', ]);
            ...
            ->addColumn('stock2', function ($row) use ($unit_data){
                if($row->enable_stock == 1) {
                    // Get the base unit and the stock
                    $stock = (float)$row->stock;
                    $baseUnit = \DB::table('units')->where('id', $row->unit_id)->first();
                    ...
                    $sub_unit_ids = !empty($row->sub_unit_ids) ? json_decode($row->sub_unit_ids, true) : [];
                    $all_units = \DB::table('units')
                        ->whereIn('id',$sub_unit_ids) // Include base unit
                        ->orderBy('base_unit_multiplier', 'asc')
                        ->get();
        
                    // Add columns dynamically for each unit
                    foreach ($all_units as $k=>$unit) {
                        $multiplier = (float)$unit->base_unit_multiplier ?: 1;
                        $unit_qty = $stock / $multiplier;
                        $unit_data[$k] = $this->transactionUtil->num_f($unit_qty, false) . ' ' . $unit->short_name;
                    }
                   // return $unit_data;
                  
                } else {
                    return ['--'];  // If stock is not enabled
                }
            })
                // Dynamically add new columns for each unit
                ->addColumn('base_unit', function ($row) use ($unit_data){
                    $units_data=$this->getUnitData($row);
                    if (isset($units_data[0]['f'])) {
                       // return $units_data[0];
                        return '<span  class="base_unit" data-orig-value="'.$units_data[0]['uf'] .'">'.$units_data[0]['f'].'</span> ';

                    }
                    return '<span  class="base_unit" data-orig-value="0">--</span> ';
                })
                ->addColumn('sub_unit_1', function ($row) {
                    $units_data=$this->getUnitData($row);
                    if (isset($units_data[1]['f'])) {
                        return $units_data[1]['f'];
                    }
                    return '--';
                })
                ->addColumn('sub_unit_2', function ($row) {
                    $units_data=$this->getUnitData($row);
                    if (isset($units_data[2]['f'])) {
                        return $units_data[2]['f'];
                    }
                    return '--';
                })
                ...
                ->editColumn('category_name', function ($row) {
                    $html= '<span class="bg bg-default">'.$row->category_name.'</span>';
                    $categories=array();
                    if(!empty($row->categories_ids)){
                        $categories=json_decode($row->categories_ids);
                    }
                    foreach($categories as $cat_id){
                    $cat_name=DB::table('categories')->where('id',$cat_id)->first()->name;
                    $html.='<span class="bg bg-default">'.$cat_name.' </span>';
                    }
                    return $html;
                })
            ...
            $raw_columns = ['unit_price', 'total_transfered', 'total_sold','base_unit',
                'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price',
                'potential_profit', 'action','category_name' ];
    ...
    /** 
     * show product accourding of group prices
     * 
     */
     public function stockPriceGroups(Request $request)
    {
        ...
        if ($request->ajax()) {
           ...
           $products=Variation::leftjoin('products', 'products.id', '=', 'variations.product_id')
            ...
            $only_price_group_only=$request->have_price_group_only;
           if($only_price_group_only){
            $products->join('variation_group_prices as vgp', 'vgp.variation_id', '=', 'variations.id')
            ->where('vgp.price_group_id', $price_group_id)
            ->where('vgp.price_inc_tax','>',0);
           }
           
            $datatable = Datatables::of($products)

                ->addColumn('action', function ($row) use($price_group_id) {
                    //button for view modal
                    return '<a data-href="/products/add-selling-price-goup/'.$row->id.'?price_group_id='.$price_group_id.'" class="btn btn-info btn-xs btn-modal" data-container=".view_modal"><i class="fa fa-pen"></i> تعديل</a>';
                ...
                ->addColumn('sell_price', function ($row) use($price_group_id) {
                    ...
                    if(!empty($variation_group_price)){
                        return  '<span class="sell_price_span hide">'.$variation_group_price->price_inc_tax.'</span><input type="number" data-url="'.action([\App\Http\Controllers\ReportController::class, 'updateSellPrice']).'" class="form-control sell_price_input" value="'.$variation_group_price->price_inc_tax.'" data-variation_id="'.$row->variation_id.'" data-price_group_id="'.$price_group_id.'">';
                    }
                    //input for update price when exprot as excel

                    return '<input type="number" class="form-control sell_price_input" value="" data-url="'.action([\App\Http\Controllers\ReportController::class, 'updateSellPrice']).'" data-variation_id="'.$row->variation_id.'" data-price_group_id="'.$price_group_id.'">';
                })
            ...
            $raw_columns = [ 'action','category_name' ,'sell_price'];
        ...
        return view('report.group_pirces_report')
            ->with(compact('selling_price_groups'));
    }
    public function updateSellPrice(Request $request){
        ...
        ->where('price_group_id', $price_group_id)->first();
        
        if(!empty($variation_group_price)){
            $variation_group_price->price_inc_tax=$price_inc_tax;
            $variation_group_price->save();
        }else{
        ...
        return response()->json(['success'=>true, 'msg'=>'تم تعديل السعر بنجاح']);
    }
    /**
    ...
    public function getproductSellReportEta(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('sell_qty', function ($row) {
                    ...
                    return '<span class="'.$class.'"  data-orig-value="'.$row->sell_qty.'" 
                    data-unit="'.$row->unit.'" >'.
                    $this->transactionUtil->num_f($row->sell_qty, false, null, true).'</span> ';
                })
                 ->editColumn('subtotal', function ($row) {
                     //ignore child sell line of combo product
                     $class = is_null($row->parent_sell_line_id) ? 'row_subtotal' : '';
                ...
                ->addColumn('tax_sub_type', function ($row) {
                    return 'W004';
                 ...
                 ->addColumn('currencey', function ($row) {
                    return 'EGP';
                 ...
                 ->addColumn('reference_oldUUID', function ($row) {
                    return null;
        ...
        return view('report.product_sell_report_eta')
            ->with(compact('business_locations', 'customers', 'categories', 'brands',
                'customer_group', 'product_custom_field1', 'product_custom_field2'));
    }
