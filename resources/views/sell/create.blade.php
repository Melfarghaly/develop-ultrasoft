...
<style>
	td input.form-control.pos_unit_price.input_number {
        display: none;
    }
	td input.form-control.row_discount_amount.input_number {
        display: none;
    }
	td select.form-control.row_discount_type {
        display: none;
    }
</style>
@endif
...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
				...
				<div class="col-md-3">
		          <div class="form-group">
					...
					@endphp
		              ...
		              <br/>
		              {!! Form::number('pay_term_number', !empty($walk_in_customer['pay_term_number']) ? $walk_in_customer['pay_term_number'] : null , ['class' => 'form-control width-40 pull-left', 'placeholder' => __('contact.pay_term'), 'required' => $is_pay_term_required]); !!}

		              {!! Form::select('pay_term_type', 
		              	['months' => __('lang_v1.months'), 
		              		'days' => __('lang_v1.days')], 
		              		!empty($walk_in_customer['pay_term_type']) ? $walk_in_customer['pay_term_type'] : null, 
	...
	@endphp
	@if((empty($status) || (!in_array($status, ['quotation', 'draft'])) || $is_enabled_download_pdf) && $sale_type != 'sales_order' || $sub_type!='permission')
		@can('sell.payments')
			@component('components.widget', ['class' => 'box-solid '.$hide_for_permission, 'id' => $payment_body_id, 'title' => __('purchase.add_payment')])
			@if($is_enabled_download_pdf)
				<div class="well row">
					<div class="col-md-6">
						<div class="form-group">
			...
			@endif
			@if(empty($status) || !in_array($status, ['quotation', 'draft']) || $sub_type!='permission')
