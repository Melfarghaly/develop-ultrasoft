...
@section('title', __('constructions::lang.add_work_certificate'))
...
<style>
    ...
    td input.form-control.input-sm.inline_discounts.input_number {
        display: none;
    }
    td input.form-control.input-sm.default_sell_price.input_number {
        display: none;
    }
    td input.form-control.input-sm.profit_percent.input_number {
        display: none;
    }
	td input.form-control.input-sm.purchase_unit_cost_without_discount.input_number {
        display: none;
    }
	td input.form-control.input-sm.purchase_unit_cost.input_number {
        display: none;
    }
</style>
@endif
...
<section class="content-header hide">
    <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">
		@if($type=='permission')
		<span class="tw-text-gray-500 tw-font-normal">({{ __('purchase.permission')}})
		@endif
		<i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="@include('purchase.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title="" title=""></i></h1>
</section>
...
<section class="content">
	...
	{!! Form::open(['url' => route('constructions.work-certificates.store'), 'method' => 'post', 'id' => 'add_purchase_form', 'files' => true ]) !!}
	@component('components.widget', ['class' => 'box-primary'])
		<div class="row">
			<div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
				<div class="form-group">
					{!! Form::label('supplier_id', __('constructions::lang.contractor') . ':*') !!}
			...
			<div class="col-sm-3  hide">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::number('exchange_rate', 1, ['class' => 'form-control', 'required', 'step' => 0.001]); !!}
					</div>
					<span class="help-block text-danger">
						@lang('purchase.diff_purchase_currency_help', ['currency' => ''])
					</span>
				</div>
			</div>
	...
	@component('components.widget', ['class' => 'box-primary'])
		<div class="row @if($type=='permission') hide @endif">
			<div class="col-sm-12">
			<table class="table">
				<tr>
					<td class="col-md-3">
	...
	@component('components.widget', ['class' => 'box-primary', 'title' => __('purchase.add_payment')])
		<div class="box-body payment_row @if($type=='permission') hide @endif">
			<div class="row">
				<div class="col-md-12">
	...
	@endcomponent
	<!--__p_currency_decimal_separator hidden field-->
<!-- Page level currency setting -->
<input type="hidden" id="p_code" value="{{$currency_details->code}}">
<input type="hidden" id="p_symbol" value="{{$currency_details->symbol}}">
<input type="hidden" id="p_thousand" value="{{$currency_details->thousand_separator}}">
<input type="hidden" id="p_decimal" value="{{$currency_details->decimal_separator}}">
	<!-- hide is_work_certificate field -->
	{!! Form::hidden('is_work_certificate', 1, ['id' => 'is_work_certificate']) !!}
{!! Form::close() !!}
</section>
<!-- quick product modal -->
...
@section('javascript')
	<script src="{{ asset('js/constructions.js?v=' . $asset_v) }}"></script>
	...
	<script type="text/javascript">
    	...
    	$(document).on('change', '.payment_types_dropdown, #location_id', function(e) {
			...
			payment_row.find('.cash-account-selection').hide();
			payment_row.find('.bank-selection').hide();
			//accounting module sync
			if(payment_type=='cash'){
				payment_row.find('.cash-account-selection').show();
				payment_row.find('.bank-selection').hide();
			} else if (['card', 'bank_transfer', 'cheque'].includes(payment_type)) {
				payment_row.find('.cash-account-selection').hide();
				payment_row.find('.bank-selection').show();
			}
