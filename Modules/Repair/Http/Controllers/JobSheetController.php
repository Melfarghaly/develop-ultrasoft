...
class JobSheetController extends Controller
{
    ...
    public function getCustomerDevices(){
        ...
        $devices = JobSheet::where('business_id', $business_id)
                ->where('contact_id', $customer_id)
                ->select(
                    'repair_job_sheets.*',
                    DB::raw("(SELECT name FROM categories WHERE id = repair_job_sheets.device_id) as device_name"),
                    DB::raw("(SELECT name FROM brands WHERE id = repair_job_sheets.brand_id) as brand_name")
                )->get();
