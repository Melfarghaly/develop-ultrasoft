$(document).ready(function() {
    ...
    $(document).on('change', '.purchase_quantity', function() {
        ...
        var implementation_rate = __read_number(row.find('input.purchase_implementation_rate'), true) / 100;

        //Calculate sub totals
        var sub_total_before_tax = quantity * purchase_before_tax * implementation_rate;
        var sub_total_after_tax = quantity * purchase_after_tax * implementation_rate;
    ...
    $(document).on('change', '.purchase_implementation_rate', function() {
        var row = $(this).closest('tr');
        var quantity = __read_number(row.find('input.purchase_quantity'), true);
        var purchase_before_tax = __read_number(row.find('input.purchase_unit_cost'), true);
        var purchase_after_tax = __read_number(
        ...
        var implementation_rate = __read_number($(this), true) / 100;
    ...
    $(document).on(
        ...
        '#purchase_list_filter_location_id, \
                    #purchase_list_filter_supplier_id,#cost_center_id, #purchase_list_filter_payment_status,\
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        var is_work_certificate = $('#is_work_certificate').val();
        var type = $('#type').val();
        var data = { 
            ...
            type: type,
            is_work_certificate: is_work_certificate
        };
...
function append_purchase_lines(data, row_count, trigger_change = false) {
    $(data)
        ...
        .each(function() {
            ...
            //Check if multipler is present then multiply it when a new row is added. latest
...
function update_purchase_entry_row_values(row) {
    if (typeof row != 'undefined') {
        ...
        var implementation_rate = __read_number(row.find('.purchase_implementation_rate'), true) / 100;
        var row_subtotal_before_tax = quantity * unit_cost_price * implementation_rate;
        ...
        var row_subtotal_after_tax = quantity * unit_cost_price_after_tax * implementation_rate;
