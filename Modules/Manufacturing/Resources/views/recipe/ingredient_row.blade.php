<tr>
	...
	<td>
		...
		{!! Form::text('ingredients[' . $row_index . '][percent_quantity]', !empty($ingredient->percent_quantity) ? @format_quantity($ingredient->percent_quantity) : 0, [
			'class' => 'form-control input-sm percent_quantity',
			'placeholder' => __('lang_v1.percent_quantity')
		]) !!} %
	...
	@php
		...
		$unit_price = $ingredient->dpp_inc_tax;
		$price = $price * $ingredient->multiplier;
		
	@endphp
	<td><span class="ingredient_price_unit">{{@num_format($unit_price)}}</span></td>
