	...
	<div class="col-md-12">
		<table class="table table-condensed">
			<tr>
				...
				<td>
					@php 
					$company = \App\ChargeCompany::where('business_id', auth()->user()->business_id)->get();
					@endphp
					<select name="company_id" class="form-control">
						<option value="">@lang('اختر شركة الشحن')</option>
						@foreach($company as $c)
							<option @if(isset($transaction->company_id) && $transaction->company_id == $c->id) selected @endif value="{{$c->id}}">{{$c->name}}</option>
						@endforeach
					</select>
				</td>
			</tr>
