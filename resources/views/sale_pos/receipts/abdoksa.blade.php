...
<div class="row" style="color: #000000 !important;">
        ...
        <table class="invoice-table">
            ...
            <td class="value">
                {{ date('H:i:s',strtotime($receipt_details->invoice_date)) }}
            </td>
                <td class="label">Time</td>
            </tr>
            <tr>
            <td class="label">تستحق في</td>
            <td class="value">
            ...
            <tr class="hide">
            <td class="label  hide">كاشير</td>
            <td class="value hide">
            {{ $receipt_details->transaction->created_by->name ?? '' }}
            </td>
                
            
                <td class="label hide">Cashier</td>
            </tr>
        </table>
    </div>
    <div class="section">
      <table class="invoice-table">
        ...
        <tr>
          <td class="label" colsapn="2">{{ $receipt_details->transaction->contact->name }}</td>
         
        <tr>
          <td class="label">رقم ضريبي</td>
          <td class="value">{{ $receipt_details->transaction->contact->tax_number_1 }}</td>
        </tr>
        <tr>
          <td class="label" colsapn="2">  {{ $receipt_details->transaction->contact->contact_address }} </td>
          
        </tr>
        <tr>
          <td class="label">{{ $receipt_details->transaction->contact->mobile }}</td>
        </tr>
      </table>
    </div>
...
<div class="row" style="color: #000000 !important;">
    ...
    <thead>
      <tr>
        <th>رقم الصنف<br>Item No</th>
        <th>اسم الصنف<br>Title</th>
        <th>الوحدة<br>Unit</th>
        <th>الكمية<br>Qty</th>
        <th>السعر<br>Price</th>
        <th>الإجمالي<br>Total</th>
        <th>الضريبة<br>Tax</th>
        <th>الإجمالي بالضريبة<br>Grand Total</th>
      </tr>
    </thead>
    <tbody>
        @php
    ...
    @forelse($receipt_details->lines as $k=> $line)
					<tr>
						...
						@endif
						<td class="text-right">{{$total_after_tax+=$line['line_total_uf']}}</td>
					</tr>
  ...
  <table class="summary-table" style>
    ...
    <td>Total</td><td>الإجمالي </td>
      ...
      <tr>
      <td>Bal. Prior</td><td>الرصيد السابق</td>
        <td>{{$receipt_details->all_due}}</td>
      </tr>
      <tr>
      <td>Bal. Due</td><td>الرصيد الحالي</td>
