...
@section('content')
    ...
    <section class="content-header no-print">
        <h1  class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">{{ $title ?? '' }}
        </h1>
    </section>
    ...
    <section class="content no-print">
        @component('components.filters', ['title' => __('report.filters')])
            ...
            @php 
                $cost_centers = [];
                $cost_centers = App\CostCenter::forDropdown(auth()->user()->business_id, true);
            @endphp
            @if ($cost_centers)
                <div class="col-md-3">
                    <div class="form-group">
                        {!! Form::label('cost_center_id', __('lang_v1.cost_center') . ':') !!}
                        {!! Form::select('cost_center_id', $cost_centers, null, [
                            'class' => 'form-control select2',
                            'style' => 'width:100%',
                            'placeholder' => __('lang_v1.all'),
                        ]) !!}
            ...
            @endif
            @php 
                $companies = \App\ChargeCompany::where('business_id', auth()->user()->business_id)->pluck('name', 'id');
            @endphp
            @if ($companies)
                <div class="col-md-3">
                    <div class="form-group">
                        {!! Form::label('company_id', __('شركة الشحن') . ':') !!}
                        {!! Form::select('company_id', $companies, null, [
                            'class' => 'form-control select2',
                            'style' => 'width:100%',
                            'id' => 'company_id',
                            'placeholder' => __('اختر شركة الشحن'),
                        ]) !!}
                    </div>
                </div>
            @endif
            <input type="hidden" name="sub_type" id="sub_type" value="{{$sub_type ?? '' }}">
            @if (!empty($sources))
                <div class="col-md-3">
                    <div class="form-group">
                ...
                @if($sub_type=='permission')
                    <div class="box-tools">
                        <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right"
                            href="{{ action([\App\Http\Controllers\SellController::class, 'create'],['type'=>'permission']) }}">
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change',
                '#sell_list_filter_location_id,#cost_center_id,#sell_list_filter_customer_id, #sell_list_filter_payment_status, #created_by, #sales_cmsn_agnt, #service_staffs, #shipping_status, #sell_list_filter_source, #payment_method, #company_id',
                function() {
                    sell_table.ajax.reload();
                });
