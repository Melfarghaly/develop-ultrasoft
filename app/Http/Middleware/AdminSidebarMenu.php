                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.opening_stock')) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseController::class, 'openingStock']),
                                __('lang_v1.add_opening_stock'),
                                ['icon' => '', 'active' => request()->segment(1) == 'add-opening-stock']
                            );
                        }
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'getproductSellReportEta']),
                            __('مبيعات فاتورة الالكترونية'),
                            ['icon' => '', 'active' => request()->segment(2) == 'product-sell-report-eta']
                        );
                        $sub->url(
            ...
            */
            ...
            $menu->dropdown(
                ...
                function ($sub) use ($enabled_modules, $is_admin) {
                    if (auth()->user()->can('stock.costprice')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'getStockReport'], ['type' => 'costprice']),
                            __('lang_v1.stock_report_cost_price'),
                            ['icon' => '', 'active' => request()->segment(2) == 'stock-report?type=costprice']
                        );
                    }
                    if (auth()->user()->can('stock.sellprice')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'getStockReport'], ['type' => 'sellprice']),
                            __('lang_v1.stock_report_sell_price'),
                            ['icon' => '', 'active' => request()->segment(2) == 'stock-report?type=sellprice']
                        );
                    }
                    if (auth()->user()->can('stock.sellprice')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'getStockReport'], ['type' => 'withoutprice']),
                            __('lang_v1.stock_report_withoutprice'),
                            ['icon' => '', 'active' => request()->segment(2) == 'stock-report?type=withoutprice']
                        );
                    }
                    if (auth()->user()->can('sell.permission')) {
                        $sub->url(
                            action([\App\Http\Controllers\SellController::class, 'index'], ['type' => 'permission']),
                            __('sale.permission'),
                            ['icon' => '', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == null]
                        );
                    }
                    if (auth()->user()->can('purchase.permission')) {
                        $sub->url(
                            action([\App\Http\Controllers\PurchaseController::class, 'index'], ['type' => 'permission']),
                            __('purchase.permission'),
                            ['icon' => '', 'active' => request()->segment(1) == 'purchases' && request()->segment(2) == null]
                        );
                    }
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                            $sub->url(
                                ...
                                ['icon' => '', 'active' => request()->segment(2) == 'stock-reportf']
                            );
                        ...
                        if (auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'stockPriceGroups']),
                                __('قوائم الاسعار'),
                                ['icon' => '', 'active' => request()->segment(2) == 'stock-price-groups']
                            );
                        if ($is_admin) {
                            $sub->url(
...
*/
            ...
            $menu->url(action([\App\Http\Controllers\CostCenterController::class, 'index']), __('مراكز التكلفة'), ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                <path d="M3 7a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v10a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-10z"></path>
                <path d="M3 7l9 6l9 -6"></path>
              </svg>', 'active' => request()->segment(1) == 'cost-center'])->order(45); //Notification template menu
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('invoice_settings.access')) {
                            ...
                            $sub->url(
                                ...
                                __('شركات الشحن'),
                                ['icon' => '', 'active' => in_array(request()->segment(1), ['companies'] )]
            ...
            // General accounts
            if (
                auth()->user()->can('access_checks') ||
                auth()->user()->can('access_vouchers') ||
                auth()->user()->can('access_cash_transactions')
            ) {
                $menu->dropdown(
                    __('general_accounts.general_accounts'),
                    function ($sub) use ($enabled_modules) {
                        // Check links
                        if (auth()->user()->can('access_checks')) {
                            $sub->url(
                                route('checks.index'),
                                __('general_accounts.checks'),
                                ['icon' => '', 'active' => request()->segment(1) == 'checks']
                            );
                            $sub->url(
                                route('checks.create.issued'),
                                __('general_accounts.create_issued_check'),
                                ['icon' => '', 'active' => request()->segment(1) == 'checks' && request()->segment(2) == 'issued']
                            );
                            $sub->url(
                                route('checks.create.received'),
                                __('general_accounts.create_received_check'),
                                ['icon' => '', 'active' => request()->segment(1) == 'checks' && request()->segment(2) == 'received']
                            );
                            $sub->url(
                                route('checks.movementChecks'),
                                __('general_accounts.checks_movement'),
                                ['icon' => '', 'active' => request()->segment(1) == 'check-transactions']
                            );
                            $sub->url(
                                route('checks.report'),
                                __('general_accounts.check_reports'),
                                ['icon' => '', 'active' => request()->segment(1) == 'check-reports']
                            );
                        }

                        // Vouchers links
                        if (auth()->user()->can('access_vouchers')) {
                            $sub->url(
                                route('vouchers.index'),
                                __('general_accounts.vouchers'),
                                ['icon' => '', 'active' => request()->segment(1) == 'vouchers']
                            );
                            $sub->url(
                                route('vouchers.create.cash_disbursement'),
                                __('general_accounts.create_cash_disbursement_voucher'),
                                ['icon' => '', 'active' => request()->segment(1) == 'vouchers' && request()->segment(2) == 'cash_disbursement']
                            );
                            $sub->url(
                                route('vouchers.create.cash_receipt'),
                                __('general_accounts.create_cash_receipt_voucher'),
                                ['icon' => '', 'active' => request()->segment(1) == 'vouchers' && request()->segment(2) == 'cash_receipt']
                            );
                            $sub->url(
                                route('vouchers.report'),
                                __('general_accounts.vouchers_report'),
                                ['icon' => '', 'active' => request()->segment(1) == 'vouchers' && request()->segment(2) == 'report']
                            );
                        }

                        // Cash transaction links
                        if (auth()->user()->can('access_cash_transactions')) {
                            $sub->url(
                                route('cash_transactions.index'),
                                __('general_accounts.cash_transactions'),
                                ['icon' => '', 'active' => request()->segment(1) == 'cash_transactions']
                            );
                            $sub->url(
                                route('cash_deposits.create'),
                                __('general_accounts.create_cash_deposit'),
                                ['icon' => '', 'active' => request()->segment(1) == 'cash_deposits' && request()->segment(2) == 'create']
                            );
                            $sub->url(
                                route('cash_withdrawals.create'),
                                __('general_accounts.create_cash_withdrawal'),
                                ['icon' => '', 'active' => request()->segment(1) == 'cash_withdrawals' && request()->segment(2) == 'create']
                            );
                            $sub->url(
                                route('cash_transactions.report'),
                                __('general_accounts.bank_transactions_report'),
                                ['icon' => '', 'active' => request()->segment(1) == 'bank_transactions_sreport']
                            );
