...
<div id="loading-spinner" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(255, 255, 255, 0.7); z-index:9999; text-align:center;">
    <div style="position:relative; top:50%; transform:translateY(-50%);">
        <i class="fa fa-spinner fa-spin fa-3x fa-fw" style="color:#333;"></i>
        <p>Loading...</p>
    </div>
</div>
...
<section class="content">
    ...
    {!! Form::open(['action' => '\Modules\Repair\Http\Controllers\JobSheetController@store', 'id' => 'job_sheet_form', 'method' => 'post', 'files' => true]) !!}
        ...
        <div class="row">
            ...
            <div class="col-sm-4 " id="customer-devices" style="display:none">
                <div class="form-group">
                    {!! Form::label('customer_device_id', __('repair::lang.device') . ':') !!}
                    <div class="input-group">
                        {!! Form::select('customer_device_id', [], null, ['class' => 'form-control ', 'id' => 'customer_device_id', 'placeholder' => __('messages.please_select')]); !!}
                    </div>
                </div>
            </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
         $('#customer_id').on('select2:select', function(e) {
            ...
            // Send an AJAX request to fetch devices for the selected customer
            $.ajax({
                type: 'GET',
                url: '/repair/job-sheets/get-customer-devices',
                data: { customer_id: customer_id },
                success: function(data) {
                    // Show the customer devices dropdown
                    $('#customer-devices').fadeIn();

                    // Reference the customer device dropdown
                    var deviceSelect = $('#customer_device_id');

                    // Clear existing options and add the default placeholder
                    deviceSelect.empty();
                    deviceSelect.append('<option value="">please_select</option>');
                    ...
                    data.forEach(function(device) {
                        deviceSelect.append(
                            `<option value="${device.id}" 
                                ...
                                data-job_sheet_no="${device.job_sheet_no || ''}" 
                                data-device_model_id="${device.device_model_id || ''}" 
                                data-device_id="${device.device_id || ''}" 
                                data-brand_id="${device.brand_id || ''}" 
                                data-status_id="${device.status_id || ''}">
                                ${device.brand_name || 'Unknown Brand'} - ${device.device_name || 'Unknown Device'} - ${device.serial_no || 'N/A'}
                            </option>`
                        );
                    });
                },
         ...
         // Populate input fields when a device is selected
         $(document).ready(function () {
            $('#customer_device_id').change(function () {
                ...
                $('#device_id').val(selectedOption.data('device_id') || '').change();
                $('#brand_id').val(selectedOption.data('brand_id') || '').change();
                setTimeout(function () {
                    $('#device_model_id').val(selectedOption.data('device_model_id') || '').change();
                }, 1500);

                setTimeout(function () {
                    $('#status_id').val(selectedOption.data('status_id') || '');

                    // Hide the spinner after everything is updated
