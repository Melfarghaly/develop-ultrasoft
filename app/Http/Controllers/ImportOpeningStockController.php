    ...
    public function storeOpeningStock(Request $request)
    {
        ...
        try {
         ...
         //   if ($request->purchases) {
                ...
                $imported_data=[];
                foreach($request->purchases as $k=>$row){
                    $imported_data[$k][0]=$row['variation_id'];
                    $imported_data[$k][1]=$request->location_id;
                    $imported_data[$k][2]=$row['quantity'];
                    $imported_data[$k][3]=$this->productUtil->num_uf( $row['purchase_price']);
                    $imported_data[$k][4]=null;
                    $imported_data[$k][5]=null;
                    $imported_data[$k][6]=null;
                }
               // dd($imported_data);
                DB::beginTransaction();
                foreach ($imported_data as $key => $value) {
                    ...
                    if (! empty($value[0])) {
                        $sku = $value[0];
                        $product_info = Variation::where('variations.id', $sku)
                                ->join('products AS P', 'variations.product_id', '=', 'P.id')
                                ->leftjoin('tax_rates AS TR', 'P.tax', 'TR.id')
                                ->where('P.business_id', $business_id)
                                ->select(['P.id', 'variations.id as variation_id',
                    ...
                    if (! empty(trim($value[1]))) {
                        $location_name = trim($value[1]);
                        $location = BusinessLocation::where('id', $location_name)
                                            ->where('business_id', $business_id)
                                            ->first();
                        if (empty($location)) {
                            $is_valid = false;
