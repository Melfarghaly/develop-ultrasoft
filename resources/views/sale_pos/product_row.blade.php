...
if($sub_type=='permission')
			{
				$product->unit_price_before_discount=0;
...
<td>
		...
		<span>سعر الشرا: @format_currency($product->default_purchase_price)</span>
		@endcan
	...
	<td>
		...
		@if(count($sub_units) > 0)
			...
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit">
				...
				if(!empty($group_prices->price_per_unit[$key])) {
					$price_per_unit = $group_prices->price_per_unit[$key];
				} else {
					$price_per_unit = null;
				}
				@endphp
                    <option value="{{$key}}" data-multiplier="{{$value['multiplier']}}" data-price-per-unit="{{$price_per_unit}}" data-unit_name="{{$value['name']}}" data-allow_decimal="{{$value['allow_decimal']}}" @if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key) selected @endif>
                        {{$value['name']}}
                    </option>
                @endforeach
           </select>
