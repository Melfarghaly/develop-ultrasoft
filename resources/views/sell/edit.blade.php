...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12">
			@component('components.widget', ['class' => 'box-solid'])
				...
				@endif
				<div class="col-md-3">
					@includeIf('cost_center.costCenter', [ 'selected_cost_center' => $transaction->cost_center_id])
				</div>
					...
					<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
						...
						<tbody>
							@foreach($sell_details as $sell_line)
								@include('sale_pos.product_row', ['product' => $sell_line, 'row_count' => $loop->index + 1, 'tax_dropdown' => $taxes, 'sub_units' => !empty($sell_line->unit_details) ? $sell_line->unit_details : [], 'action' => 'edit', 'is_direct_sell' => true, 'so_line' => $sell_line->so_line, 'is_sales_order' => $transaction->type == 'sales_order'])
							@endforeach
						</tbody>
					</table>
					</div>
	...
	@if($transaction->type = 'sell' && $sub_type !='permission')
	@can('sell.payments')
		@component('components.widget', ['class' => 'box-solid', 'title' => __('purchase.add_payment')])
		<div class="row">
			@foreach($payment_lines as $payment_line)			
        		...
        		@endif
				<div class="payment_row @if(!empty($payment_line['parent_id']) ) hide @endif" id="payment_rows_div">
