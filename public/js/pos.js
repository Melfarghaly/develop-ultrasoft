...
$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'select.tax_id', function() {
        ...
        // Get the selected tax rate
            ...
            .data('rate');
debugger;
        // Get the current unit price (excluding tax)
        var unit_price = __read_number(tr.find('input.pos_unit_price'));

        // Calculate the new unit price including tax based on the tax rate pos_unit_price_inc_tax
       //if tax rate undefined then set tax rate 0
       if (!tax_rate) {
           tax_rate = 0;// default tax rate
           }
        var unit_price_inc_tax = unit_price * (1 + (tax_rate / 100));

        // Write the new unit price including tax back to the input
        __write_number(tr.find('input.pos_unit_price_inc_tax'), unit_price_inc_tax);
        tr.find('input.pos_unit_price_inc_tax').trigger('change');
        pos_each_row(tr);
    });
...
debugger;   
        ...
        var price_per_unit = parseFloat(selected_option.data('price-per-unit'));
        if (price_per_unit) {
            unit_sp = price_per_unit 
        }
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
    ...
    $.ajax({
        ...
        dataType: 'html',
        success: function(result) {
            $('div#product_list_body').prepend(result);
            $('#suggestion_page_loader').fadeOut(700);
        },
    });
}
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    // Since the row is added at the beginning, get the first row instead of the last
                    ...
                    // Check if multiplier is present then multiply it when a new row is added
                    ...
                    // Optional: Scroll to top instead of bottom since the new row is at the top
