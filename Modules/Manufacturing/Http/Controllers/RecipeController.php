        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($recipes)
                ...
                ->editColumn('total_quantity', function ($row) {
                    ...
                    $html = '<span class="display_currency is_quantity" data-currency_symbol="false" data-is_quantity="true">'.$quantity.'</span>';
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (! empty($input['ingredients'])) {
                ...
                $unit_price= $this->moduleUtil->num_uf($input['total']) / $this->moduleUtil->num_uf($input['total_quantity']);

                if (! empty($request->input('sub_unit_id'))) {
                    $sub_unit = \App\Unit::find($request->input('sub_unit_id'));
                    if($sub_unit->base_unit_multiplier != 0){
                        $unit_price = $unit_price / $sub_unit->base_unit_multiplier;
                    }
                }
                ...
                foreach ($input['ingredients'] as $key => $value) {
                    ...
                    $ingredient->percent_quantity = $this->moduleUtil->num_uf($value['percent_quantity']);
                   // dd($this->moduleUtil->num_uf($value['quantity']));
        ...
        if (! empty($recipe) || (empty($recipe) && ! empty($copy_recipe))) {
            ...
            foreach ($ingredients_obj as $ingredient) {
                ...
                $temp = [
                    ...
                    'percent_quantity' => ! empty($ingredient->percent_quantity) ? $ingredient->percent_quantity : 0,
