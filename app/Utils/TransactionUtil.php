...
class TransactionUtil extends Util
{
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (! empty($payment['payment_id'])) {
                ...
                $transaction_payment = TransactionPayment::find($payment['payment_id']);
                ...
                if ($payment_amount != 0) {
                    ...
                    if($payment['method']=='cash'){
                        $payment['accounting_account_id']=$payment['cash_account_id'] ?? null;
                    }elseif(in_array($payment['method'],['cheque','bank_transfer','card'])){
                        $payment['accounting_account_id']=$payment['bank_account_id'] ?? null;
                    }
                    $payment_data = [
                        ...
                        'accounting_account_id' => ! empty($payment['accounting_account_id']) && $payment['method'] != 'advance' ? $payment['accounting_account_id'] : null,
                    ];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['tax_uf'] = ($transaction->tax_amount != 0) ? $transaction->tax_amount : 0;
        ...
        if (! empty($il->module_info['repair']) && $transaction->sub_type == 'repair') {
            ...
            $job_sheet=DB::table('repair_job_sheets')->where('id',$transaction->repair_job_sheet_id)->first();
            if(!empty( $job_sheet)){
                $output['repair_custom_field1'] = $job_sheet->custom_field_1 ?? '' ;
                $output['repair_job_sheet_date'] = $job_sheet->created_at ?? '' ;
            }
         //  dd($job_sheet);
    ...
    {
        // نجلب كل الفواتير مع مجموع السطور وقيمة الخصم ونوعه
        $rows = TransactionSellLine::leftJoin('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.status', 'final')
            // صلاحيات المستخدم على المواقع
            ->when(auth()->user()->permitted_locations() != 'all', function($q) {
                $q->whereIn('t.location_id', auth()->user()->permitted_locations());
            })
            // فلترة بالتاريخ
            ...
            // فلترة بوكيل العمولة
            ->when($commission_agent, function($q) use ($commission_agent) {
                $q->where('t.commission_agent', $commission_agent);
            })
            ->select(
                't.id as transaction_id',
                't.discount_amount',
                't.discount_type',
                DB::raw('SUM((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price) as line_total')
            )
            ->groupBy('t.id', 't.discount_amount', 't.discount_type')
            ->get();
        ...
        foreach ($rows as $row) {
            ...
            if ($row->discount_type === 'percentage') {
                $discount_value = $row->line_total * ($row->discount_amount / 100);
            } else {
                // خصم ثابت
                $discount_value = $row->discount_amount;
            }

            // نطرح الخصم من مجموع السطور
            if($row->line_total != 0) {
                $discount_value = $row->discount_amount;
            } else {
                $discount_value = 0;
            }
            $net = $row->line_total - $discount_value;
//dd($net, $row->line_total, $row->discount_amount, $row->discount_type);
            // نجمع الصافي لكل فاتورة
            $total_for_commission += $net;
        }

        return [
            'total_sales_with_commission' => $total_for_commission
        ];
    }

    public function getTotalSellCommissionOld($business_id, $start_date = null, $end_date = null, $location_id = null, $commission_agent = null)
    {
        ...
        $query = TransactionSellLine::leftjoin('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
                            ...
                            ->where('t.status', 'final')
                            //
                            ->select('transactions.discount_amount','transactions.discount_type', DB::raw('SUM( (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price ) as final_total'));
        //ignore discount when calc commision
        //Check for permitted locations of a user
        $permitted_locations = auth()->user()->permitted_locations();
        if ($permitted_locations != 'all') {
            $query->whereIn('t.location_id', $permitted_locations);
    ...
    public function isCustomerCreditLimitExeeded(
        ...
        if (! empty($input['payment']) && ! $is_credit_sale) {
            foreach ($input['payment'] as $payment) {
                $curr_total_payment += $this->num_uf($payment['amount'] ?? 0);
            }
        }
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        //add jornal entry transactions
        $joranl_entries=$this->__jornalEntryQuery($contact_id, $start, $end, $location_id);
       if(!empty($joranl_entries))
        foreach($joranl_entries as $joranl){
            $ledger[] = [
                'date' => $joranl->operation_date,
                'ref_no' => $joranl->ref_no,
                'type' => $joranl->mapping_type=='journal_entry' ? __('accounting::lang.journal_entry') : 'قيد افتتاحي',
                'location' => '',
                'payment_status' => '',
                'total' =>'',
                'payment_method' => '',
                'debit' => $joranl->type == 'debit' ? $joranl->amount : '',
                'credit' => $joranl->type == 'credit' ? $joranl->amount : '',
                'others' => $joranl->note.' '.$joranl->operation_date,
        ...
        $output = [
            ...
            'balance_due' => $final_balance,//$curr_due,
    ...
    private function __jornalEntryQuery($contact_id,$start_date,$end_date){
        ...
        $transactions = AccountingAccountsTransaction::where('accounting_accounts_transactions.accounting_account_id', $account->id)
                        ->leftjoin('accounting_acc_trans_mappings as ATM', 'accounting_accounts_transactions.acc_trans_mapping_id', '=', 'ATM.id')
                        ...
                        ->whereNull('accounting_accounts_transactions.transaction_id')
                        ->whereNull('accounting_accounts_transactions.transaction_payment_id')
                        ->whereNull('accounting_accounts_transactions.voucher_id')
                        ->whereNull('accounting_accounts_transactions.check_id')
                        ->where('accounting_accounts_transactions.sub_type','!=','opening_balance_old')
                        ->whereNull('accounting_accounts_transactions.cash_transaction_id')
                        ->select('accounting_accounts_transactions.operation_date',
                            ...
                            'ATM.ref_no', 'ATM.note','accounting_accounts_transactions.note as operation_note',
    ...
    public function payContactVocher($trasnsaction_type,$id,$contact, $type=null,$amount)
    {
        $inputs=[];
        $voucher_id=null;
        $check_id=null;
        $method='cash';
        $cashTransaction_id=null;
        //delete if exist 
        if($trasnsaction_type=='voucher'){
            $payment=TransactionPayment::where('voucher_id',$id)->first();
            $voucher_id=$id;
        }elseif($trasnsaction_type=='check'){
            $payment=TransactionPayment::where('check_id',$id)->first();
            $check_id=$id;
            $method='cheque';
        }elseif($trasnsaction_type=='cashTransaction'){
            $payment=TransactionPayment::where('cashTransaction_id',$id)->first();
            $method='bank';
            $cashTransaction_id=$id;
        }
      ...
      //  dd($contact);
        $format_data = false;
        if($contact->type=='customer' && in_array($type,['cash_receipt','received'])){
            $payment_type="sell";
            
        }elseif($contact->type=='customer' && in_array($type,['cash_disbursement','issued','deposit'])){
            $payment_type="sell_return";
        }elseif($contact->type=='supplier' && in_array($type,['cash_receipt','received'])){
            $payment_type="purchase_return";
        }elseif($contact->type=='supplier' && in_array($type,['cash_disbursement','issued','withdrawal'])){
        ...
        $inputs['payment_for'] = $contact_id;
        $inputs['business_id'] = $business_id;
        $inputs['voucher_id'] = $voucher_id;
        $inputs['check_id'] = $check_id;
        $inputs['cashTransaction_id'] = $cashTransaction_id;
        $inputs['is_advance'] = 1;
        $due_payment_type = $payment_type;//$contact->type == 'supplier' ? 'purchase' : 'sell';
        $prefix_type = '';
        if ($contact->type == 'customer') {
            $prefix_type = 'sell_payment';
     ...
     //   $this->updateContactCurrentDue($contact->id);
        return $parent_payment;
    }
        ...
        $payment = TransactionPayment::findOrFail($id);
                ...
                if (! empty($payment->transaction_id)) {
                    ...
                    if ($total_customer_advance > 0) {
                        $this->updateContactBalance($payment->payment_for, $total_customer_advance, 'deduct');
                    }
    ...
    public function addCombinedSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        $discount = [
            'discount_type' => $input['discount_type'] ?? 'fixed',
            'discount_amount' => $input['discount_amount'] ?? 0,
        ...
        if (empty($sell_return)) {
            ...
            //cost center id
            $sell_return_data['cost_center_id'] = $input['cost_center_id'] ?? null;
           //dd($sell_return_data);
            $sell_return = Transaction::create($sell_return_data);
            //dd($sell_return);
    ...
    public function get_sum_taxes_lines($business_id,$transaction_id,$is_return=false){
        $sell=Transaction::with(['sell_lines'])->find($transaction_id);
      
        $taxes = TaxRate::where('business_id', $business_id)
                                ->pluck('name', 'id');
        $service_lines=\DB::table('pjt_invoice_lines')->where('transaction_id',$transaction_id)->get();
          ...
          //  dd($sell->sell_lines);
            foreach ($sell->sell_lines as $key => $value) {
                ...
                if (!empty($taxes[$value->tax_id])) {
                    $quantity=$value->quantity;
                    if($is_return)
                        $quantity=$value->quantity_returned;
                        
                    if(isset($line_taxes[$taxes[$value->tax_id]])) {
                        $line_taxes[$taxes[$value->tax_id]] += ($value->item_tax * $quantity);
                    } else {
                        $line_taxes[$taxes[$value->tax_id]] = ($value->item_tax * $quantity);
                    }
                }
            ...
            //  dd($line_taxes);
            $taxes_line_sum=0;
            foreach($line_taxes as $key=>$v){
                $taxes_line_sum +=(float)$v;
            }
            return $taxes_line_sum;
        }else if(count($service_lines)>0){
            
             foreach ($service_lines as $key => $value) {
                if (!empty($value->sub_unit_id)) {
                    $formated_sell_line = $this->recalculateSellLineTotals($business_id, $value);
                    $sell->sell_lines[$key] = $formated_sell_line;
                }
    
                if (!empty($taxes[$value->tax_rate_id])) {
                    if(isset($line_taxes[$taxes[$value->tax_rate_id]])) {
                        $line_taxes[$taxes[$value->tax_rate_id]] += ($value->total - ($value->rate * $value->quantity));
                    } else {
                        $line_taxes[$taxes[$value->tax_rate_id]] = ($value->total - ($value->rate * $value->quantity));
                    }
                }
    ...
    public function get_purchase_taxes_lines($business_id,$transaction_id){
        $productUtil=new \App\Utils\ProductUtil();
        $purchase = Transaction::where('business_id', $business_id)
                                ->where('id', $transaction_id)
                                ->with(
        ...
        $payment_methods = $productUtil->payment_types($purchase->location_id, true);
        $purchase_taxes = [];
        if (!empty($purchase->tax)) {
            if ($purchase->tax->is_tax_group) {
                $purchase_taxes = $this->sumGroupTaxDetails($this->groupTaxDetails($purchase->tax, $purchase->tax_amount));
            } else {
                $purchase_taxes[$purchase->tax->name] = $purchase->tax_amount;
            }
        }
        $taxes_line_sum=0;
        foreach($purchase_taxes as $key=>$v){
            $taxes_line_sum +=(float)$v;
        }
        return $taxes_line_sum;
    }
    public function sumDiscounts ($id){
        ...
        if($transaction->discount_type=='fixed'){
            $discount_amount+=$transaction->discount_amount;
             ...
             $discount_amount+= ( $transaction->total_before_tax / 100) * $transaction->discount_amount ;
        }
        ...
        if($transaction->type=='sell'){
            ...
            foreach ($transaction->sell_lines  as $key => $value) {
            if (!empty($value->sub_unit_id)) {
                $formated_sell_line = $this->recalculateSellLineTotals($transaction->business_id, $value);
                $transaction->sell_lines[$key] = $formated_sell_line;
            }
            }
           foreach($transaction->sell_lines as $line){
                ...
                if($line->line_discount_type=='fixed'){
                    $discount_amount+=$line->get_discount_amount();
                       ...
                       $discount_amount+=   (($line->quantity * $line->unit_price_before_discount)/100)*$line->line_discount_amount; 
                }
           }
        }
        ...
        if($transaction->type=='purchase'){
            foreach($transaction->purchase_lines as $line){
                $discount_amount+=   (($line->quantity * $line->purchase_price)/100)*$line->discount_percent; 
