...
Route::middleware('web', 'SetSessionData', 'auth', 'language', 'timezone', 'AdminSidebarMenu')->prefix('accounting')->group(function () {
    ...
    Route::get('chart-of-account/ajax-tree', [\Modules\Accounting\Http\Controllers\CoaController::class, 'indexajax'])->name('accounting.ajax_tree');
    Route::get('chart-of-account/load-ajax-tree', [\Modules\Accounting\Http\Controllers\CoaController::class, 'treeData'])->name('accounting.accounts.tree');
    ...
    Route::get('post-balance/{account_id}', [\Modules\Accounting\Http\Controllers\CoaController::class, 'post_balance'])->name('accounting.post_balance');

    Route::resource('journal-entry', \Modules\Accounting\Http\Controllers\JournalEntryController::class);
    Route::get('opening-jornal-entry', [\Modules\Accounting\Http\Controllers\JournalEntryController::class,'createOpening']);

    Route::get('journal-entry/{id}/destroy', [\Modules\Accounting\Http\Controllers\JournalEntryController::class,'destroy']);
    Route::get('settings', [\Modules\Accounting\Http\Controllers\SettingsController::class, 'index']);
    Route::any('reset-data', [\Modules\Accounting\Http\Controllers\SettingsController::class, 'resetData']);
